...
class DatePickerModel extends CommonPickerModel {
  ...
  DatePickerModel({DateTime currentTime, DateTime maxTime, DateTime minTime, DateType dateType, LocaleType locale})
      : super(locale: locale) {
    this.maxTime = maxTime ?? DateTime(2049, 12, 31);
    this.minTime = minTime ?? DateTime(1970, 1, 1);
    this.dateType = dateType ?? DateType.Y_M_D;

    var now = DateTime.now();
    currentTime = currentTime ?? DateTime(now.year, now.month, now.day);
    if (currentTime != null) {
...
class TimePickerModel extends CommonPickerModel {
  bool showSecond;
  TimePickerModel({bool showSecond, DateTime currentTime, LocaleType locale}) : super(locale: locale) {
    this.showSecond = showSecond;
    this.currentTime = currentTime ?? DateTime.now();

    _currentLeftIndex = this.currentTime.hour;
    _currentMiddleIndex = this.currentTime.minute;
    _currentRightIndex = showSecond ? this.currentTime.second : 0;
  }
